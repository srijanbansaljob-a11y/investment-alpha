  ...
  try {
    ...
    if (fillStatus !== "filled") {
      try {
        ...
        const poll = await fetch(`https://paper-api.alpaca.markets/v2/orders/${result.id}`, { headers: headers2 });
        if (poll.ok) {
        ...
        console.warn("Fill poll error:", e.message);
      }
    }
  ...
  if (i.type === MESSAGE_COMPONENT) {
    ...
    if (action === "approve_rebalance") {
      const err = await dispatchToGitHub(env, {
        command: "approve_rebalance",
        message_id: i.message.id,
        ...common
      });
      const embeds = i.message.embeds || [];
      if (embeds[0]) {
        embeds[0].color = err ? 15158332 : 3066993;
        embeds[0].footer = { text: err ? `\u274C Approval NOT sent \u2014 ${err}` : "\u2705 Approved \u2014 executing rebalance now. Results post here in ~1\u20132 min." };
      }
      return json({ type: R_UPDATE_MESSAGE, data: { embeds, components: [] } });
    }
    if (action === "reject_rebalance") {
      await dispatchToGitHub(env, {
        command: "reject_rebalance",
        message_id: i.message.id,
        ...common
      });
      const embeds = i.message.embeds || [];
      if (embeds[0]) {
        embeds[0].color = 9807270;
    ...
    if (action === "confirm_buy_execute") {
      ...
      if (embeds[0]) {
        ...
        const fillNote = filled ? `\u2705 Filled ${result.fillQty} @ $${parseFloat(result.fillAvg || result.price).toFixed(2)}` : `\u23F3 Status: ${result.fillStatus || "submitted"} \u2014 market orders typically fill within seconds`;
        embeds[0].color = ok ? filled ? C_GREEN : 15965202 : C_RED;
        embeds[0].title = ok ? `\u2705 BUY ${ticker} \u2014 Order ${filled ? "Filled" : "Placed"}` : `\u274C BUY ${ticker} \u2014 Order Failed`;
        embeds[0].footer = { text: ok ? `${fillNote} \xB7 Stop $${result.stopPrice} \xB7 ATR ${result.atrPct ? result.atrPct.toFixed(2) + "%" : "default"}` : `Error: ${result.error}` };
      }
      return json({ type: R_UPDATE_MESSAGE, data: { embeds, components: [] } });
    }
    if (action === "tp_sell") {
...
Permitted: ${permitted.join(", ") || "none"}`);
    ...
    if (action === "confirm_sell_execute") {
      ...
      let ok = false, errMsg = "";
      try {
        const openR = await fetch(`${alpacaBase}/v2/orders?status=open&limit=50`, { headers: headers2 });
        if (openR.ok) {
          const openOrders = await openR.json();
          const toCancel = (Array.isArray(openOrders) ? openOrders : []).filter((o) => o.symbol === ticker);
          await Promise.all(toCancel.map(
            (o) => fetch(`${alpacaBase}/v2/orders/${o.id}`, { method: "DELETE", headers: headers2 }).catch(() => {
            })
          ));
        ...
        if (partial) {
          ...
          r = await fetch(`${alpacaBase}/v2/positions/${ticker}`, { method: "DELETE", headers: headers2 });
        }
      ...
      if (embeds[0]) {
        ...
        embeds[0].title = ok ? partial ? `\u2705 SELL ${ticker} \u2014 ${sellQty} shares sold` : `\u2705 SELL ${ticker} \u2014 Position Closed` : `\u274C SELL ${ticker} \u2014 Failed`;
        embeds[0].footer = { text: ok ? partial ? `${sellQty} of ${totalQty} shares sold \xB7 ${totalQty - sellQty} remain \xB7 Paper trading` : "Full position closed \xB7 Bracket legs cancelled first \xB7 Paper trading" : `Error: ${errMsg}` };
      }
      return json({ type: R_UPDATE_MESSAGE, data: { embeds, components: [] } });
    }
    if (action === "cancel") {
      ...
      const e5 = {
        ...
        fields: [
          ...
          },
          {
            ...
            value: "`/regime` check market \xB7 `/screener` see picks \xB7 `/buy symbol:X portfolio:Screener` trade\nMonthly (1st): `/pipeline mode:dry` review \xB7 `/pipeline mode:execute` rebalance",
            inline: false
          }
        ],
...
Permitted: ${permitted.join(", ") || "none"}`);
    ...
    if (name === "sell") {
      ...
      const label = partial ? `\u{1F534} Sell ${sellQty} of ${totalQty} [${portfolio === "screener" ? "Screener" : "Pipeline"}]` : `\u{1F534} Sell all ${totalQty} shares [${portfolio === "screener" ? "Screener" : "Pipeline"}]`;
      const footerTxt = partial ? `Partial sell \u2014 ${totalQty - sellQty} shares remain \xB7 Paper trading` : "Closes entire position (cancels bracket legs first) \xB7 Paper trading";

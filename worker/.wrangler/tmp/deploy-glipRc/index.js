...
__name(screenerHeaders, "screenerHeaders");
...
async function placeBracketOrder(env, symbol, customQty = null) {
  ...
  try {
    const posKey = `position_target_SCREENER_${symbol}`;
...
async function buildBuyPreview(env, ticker, customQty = null) {
  const alpacaBase = "https://paper-api.alpaca.markets";
  const headers = screenerHeaders(env);
...
Close a position before buying ${ticker}.` };
  ...
  const embed = {
    title: `\u{1F6D2} Buy Preview \u2014 ${ticker}`,
    color: C_BLUE,
    ...
    footer: { text: "Bracket order \xB7 Stop + ceiling auto-managed by Alpaca \xB7 Paper trading" }
  };
  const components = [{ type: 1, components: [
    { type: 2, style: 3, label: `\u2705 Buy ${qty} \u2014 $${totalCost.toFixed(0)}`, custom_id: `ia|confirm_buy_execute|${ticker}|buy|${qty}` },
    { type: 2, style: 2, label: "\u270F\uFE0F Change qty", custom_id: `ia|change_qty|${ticker}|buy` },
...
async function handleDiscordInteraction(bodyText, env, ctx) {
  ...
  if (i.type === MODAL_SUBMIT) {
    const [mtag, mticker] = (i.data.custom_id || "").split("|");
    if (mtag === "ia_qty_modal") {
      ...
      const preview = await buildBuyPreview(env, mticker, customQty);
  ...
  if (i.type === MESSAGE_COMPONENT) {
    const [tag, action, ticker, trigger, qtyOverride] = (i.data.custom_id || "").split("|");
    ...
    if (action === "confirm_buy_execute") {
      const result = await placeBracketOrder(env, ticker, qtyOverride ? parseInt(qtyOverride) : null);
    ...
    if (action === "tp_sell") {
      ...
      try {
        const raw = await env.KV.get("pending_take_profits");
        const pending = raw ? JSON.parse(raw) : {};
        if (pending[ticker]) {
          pending[ticker].status = ok ? "executed" : "failed";
          pending[ticker].executed_at = (/* @__PURE__ */ new Date()).toISOString();
          await env.KV.put("pending_take_profits", JSON.stringify(pending), { expirationTtl: 8 * 24 * 3600 });
        }
      } catch (e) {
    ...
    if (action === "move_stop_breakeven") {
      ...
      let targets = null;
      try {
        const tr = await env.KV.get(`position_target_SCREENER_${ticker}`);
...
Your original stop at $${currentStopPrice.toFixed(2)} should still be active.`);
      ...
      if (newStopOk) {
        ...
        await env.KV.put(`position_target_SCREENER_${ticker}`, JSON.stringify(targets), { expirationTtl: 8 * 24 * 3600 });
      }
...
Permitted: ${permitted.join(", ") || "none"}`);
      const preview = await buildBuyPreview(env, ticker);
      if (preview.error) return ephemeral(preview.error);
      return json({ type: R_CHANNEL_MESSAGE, data: { flags: EPHEMERAL, embeds: preview.embeds, components: preview.components } });
    }
...
Permitted: ${permitted.join(", ") || "none"}`);
      const preview = await buildBuyPreview(env, symbol);
      if (preview.error) return ephemeral(preview.error);
      return json({ type: R_CHANNEL_MESSAGE, data: { flags: EPHEMERAL, embeds: preview.embeds, components: preview.components } });
    }
    if (name === "sell") {
      ...
      return json({ type: R_CHANNEL_MESSAGE, data: { flags: EPHEMERAL, embeds: [{
        ...
        { type: 2, style: 4, label: `\u{1F534} Sell all ${qty} shares`, custom_id: `ia|confirm_sell_execute|${symbol}|sell` },
...
async function runMorningBrief(env) {
  ...
  const alpacaBase = "https://paper-api.alpaca.markets";
  const headers = screenerHeaders(env);
  let positions = [], account = null, regime = null, summary = null;
  try {
    const [posR, acctR, rr, sr] = await Promise.all([
      fetch(`${alpacaBase}/v2/positions`, { headers }),
      fetch(`${alpacaBase}/v2/account`, { headers }),
      env.KV.get("regime_signal"),
      env.KV.get("screener_summary")
    ]);
    if (posR.ok) positions = await posR.json();
    if (acctR.ok) account = await acctR.json();
  ...
  const totalVal = positions.reduce((s, p) => s + parseFloat(p.market_value || 0), 0);
  ...
  const posLines = positions.length ? positions.map((p) => {
    ...
    return `${pnl >= 0 ? "\u{1F4C8}" : "\u{1F4C9}"} **${p.symbol}** ${pnl >= 0 ? "+" : ""}$${pnl.toFixed(2)} (${pnlPct.toFixed(1)}%)`;
  }).join("\n") : "_No open positions_";
  ...
  await postDiscordWebhook(webhookUrl, [{
    ...
    description: posLines,
    fields: [
      { name: "Regime", value: `${regLabel} (${regScore}/100)`, inline: true },
      { name: "Positions", value: `${positions.length} / ${MAX_POSITIONS} open`, inline: true },
      { name: "Unrealised P&L", value: `${totalPnl >= 0 ? "+" : ""}$${totalPnl.toFixed(2)}`, inline: true },
      { name: "Portfolio value", value: `$${totalVal.toFixed(2)}`, inline: true },
      { name: "Buying power", value: `$${bp.toFixed(2)}`, inline: true },
...
async function runTakeProfitMonitor(env) {
  ...
  let positions = [];
  try {
    const r = await fetch(`${alpacaBase}/v2/positions`, { headers });
    if (r.ok) positions = await r.json();
  } catch (e) {
    console.error("TP monitor positions error:", e.message);
    return;
  }
  ...
  try {
    ...
    console.warn("TP KV read error:", e.message);
  }
  const webhookUrl = env.DISCORD_WEBHOOK_URL || "";
  ...
  for (const pos of positions) {
    ...
    let targets = null;
    try {
      const tRaw = await env.KV.get(`position_target_SCREENER_${symbol}`);
    ...
    if (unrealPct >= 5 && targets.stop_price < targets.entry_price && !targets.trailing_notified) {
      ...
      targets.trailing_notified = true;
      try {
        await env.KV.put(`position_target_SCREENER_${symbol}`, JSON.stringify(targets), { expirationTtl: 8 * 24 * 3600 });
      } catch (e) {
        console.warn("Trailing stop KV update:", e.message);
      }
      if (webhookUrl) {
        await postDiscordWebhook(webhookUrl, [{
          title: `\u{1F6E1}\uFE0F Trailing Stop Alert \u2014 ${symbol} up ${unrealPct.toFixed(1)}%`,
          color: C_BLUE,
          description: [
          ...
          type: 1,
          components: [
            { type: 2, style: 1, label: "\u{1F504} Move Stop to Breakeven", custom_id: `ia|move_stop_breakeven|${symbol}|trailing` }
          ]
        }]);
      }
    }
    if (currentPrice >= monitorPrice) {
      ...
      const pnlPct = (currentPrice / entryPrice - 1) * 100;
      const embed = {
        title: `\u{1F514} TAKE PROFIT ALERT \u2014 ${symbol}`,
        color: C_GREEN,
      ...
      const components = isRegularHours ? [{
        type: 1,
        components: [
          { type: 2, style: 3, label: "\u{1F4B0} Sell All", custom_id: `ia|tp_sell|${symbol}|tp` },
          { type: 2, style: 1, label: "\u{1F4CA} Sell 50%", custom_id: `ia|tp_half_sell|${symbol}|tp` },
          { type: 2, style: 2, label: "\u{1F91A} Hold", custom_id: `ia|tp_hold|${symbol}|tp` }
        ]
      }] : [];
      let messageId = null;
      if (webhookUrl) {
  ...
  if (pendingChanged) {
    try {
      ...
      console.warn("TP KV write error:", e.message);
    }
  }
}
